...
function makeSizeBreakdown(p) {
  ...
  return SIZES.map((s, i) => {
    ...
    const acceptanceRub = Math.round(p.acceptance / 100 * salesRub);
    const storageRub = Math.round(p.storage / 100 * salesRub);
    const commissionRub = Math.round(p.commission / 100 * salesRub);
    const logisticsRub = Math.round(p.logistics / 100 * salesRub);
    const adsRub = Math.round(p.ads / 100 * salesRub);
    const expensesRub = acceptanceRub + storageRub + commissionRub + logisticsRub + adsRub;
    return {
      size: s, sales, salesRub, returns, cost, marginRub, margin, prevMargin: margin - 1 - i * 0.3,
      buyout,
      buyoutPct: sales ? buyout / sales * 100 : 0,
      orderUnits: sales + returns,
      orderRub: Math.round((sales + returns) * perUnit),
      returnsRub: Math.round(returns * perUnit),
      returnsPct: sales ? returns / sales * 100 : 0,
      costPct: salesRub ? cost / salesRub * 100 : 0,
      costUnit: sales ? Math.round(cost / sales) : 0,
      logisticsRub, storageRub, adsRub, commissionRub, acceptanceRub,
      acceptance: p.acceptance, storage: p.storage, commission: p.commission, logistics: p.logistics, ads: p.ads,
      expensesRub,
      expensesPct: salesRub ? expensesRub / salesRub * 100 : 0,
...
// ── Column groups (4 groups). base:true → shown collapsed & in standard.
// pro:true → extra column revealed only when the group is expanded in Pro mode (label shown in accent/blue).
const COLUMN_GROUPS = [
  {
    id: 'revenue', label: 'Выручка',
    cols: [
      { key:'buyoutPct',  label:'Выкуп, %',     format:'pct', width:95,  numeric:true, base:true },
      { key:'sales',      label:'Выручка, шт',  format:'num', width:110, numeric:true, base:true },
      { key:'salesRub',   label:'Выручка, ₽',   format:'rub', width:130, numeric:true, base:true },
      { key:'orderUnits', label:'Продажа, шт',  format:'num', width:110, numeric:true, pro:true },
      { key:'orderRub',   label:'Продажи, ₽',   format:'rub', width:130, numeric:true, pro:true },
      { key:'returns',    label:'Возврат, шт',  format:'num', width:110, numeric:true, pro:true, invert:true },
      { key:'returnsPct', label:'Возврат, %',   format:'pct', width:105, numeric:true, pro:true, invert:true },
    ]
  },
  {
    id: 'cost', label: 'Себестоимость',
    cols: [
      { key:'cost',     label:'Себест., ₽',    format:'rub', width:120, numeric:true, base:true, invert:true },
      { key:'costPct',  label:'Себест., %',    format:'pct', width:105, numeric:true, base:true, invert:true },
      { key:'costUnit', label:'Себест. на ед.',  format:'rub', width:130, numeric:true, pro:true,  invert:true },
    ]
  },
  {
    id: 'expenses', label: 'Издержки',
    cols: [
      { key:'expensesRub',   label:'Издержки, ₽',  format:'rub', width:120, numeric:true, base:true, invert:true },
      { key:'expensesPct',   label:'Издержки, %',  format:'pct', width:110, numeric:true, base:true, invert:true },
      { key:'acceptanceRub', label:'Приём, ₽',     format:'rub', width:105, numeric:true, pro:true,  invert:true },
      { key:'acceptance',    label:'Приём, %',     format:'pct', width:100, numeric:true, pro:true,  invert:true },
      { key:'storageRub',    label:'Хранен., ₽',   format:'rub', width:115, numeric:true, pro:true,  invert:true },
      { key:'storage',       label:'Хранен., %',   format:'pct', width:105, numeric:true, pro:true,  invert:true, norm:4 },
      { key:'commissionRub', label:'Комис., ₽',    format:'rub', width:110, numeric:true, pro:true,  invert:true },
      { key:'commission',    label:'Комис., %',    format:'pct', width:100, numeric:true, pro:true,  invert:true, norm:11 },
      { key:'logisticsRub',  label:'Логист., ₽',   format:'rub', width:110, numeric:true, pro:true,  invert:true },
      { key:'logistics',     label:'Логист., %',   format:'pct', width:100, numeric:true, pro:true,  invert:true, norm:8 },
      { key:'adsRub',        label:'Реклама, ₽',   format:'rub', width:110, numeric:true, pro:true,  invert:true },
      { key:'ads',           label:'Реклама, %',   format:'pct', width:100, numeric:true, pro:true,  invert:true, norm:5 },
    ]
  },
  {
    id: 'margin', label: 'Маржа',
    cols: [
      { key:'marginRub',  label:'Маржа, ₽',    format:'rub', width:120, numeric:true, base:true },
      { key:'margin',     label:'Маржа, %',    format:'pct', width:100, numeric:true, base:true },
      { key:'marginUnit', label:'Марж/ед, ₽',  format:'rub', width:115, numeric:true, pro:true },
    ]
  },
];
...
const STATUS_RULES = {
  ...
  expensesPct:{ bad: v => v > 35,        tol: v => v > 28,                                  unit:'%', label:'Издержки' },
...
function Cell({ row, col, colStats, patterns, rank, sizeMode = false, firstInGroup = false }) {
  ...
  return React.createElement('td', {
    ...
    style: {
      ...
      ...(firstInGroup ? { borderLeft: '3px solid color-mix(in srgb, var(--text-secondary) 30%, var(--border))' } : {}),
...
function NomenclatureReportV2({ paid: initialPaid = false }) {
  ...
  const rows = React.useMemo(() => {
    let r = PRODUCTS.map(p => {
      ...
      const acceptanceRub = Math.round(p.acceptance / 100 * p.salesRub);
      const storageRub = Math.round(p.storage / 100 * p.salesRub);
      const commissionRub = Math.round(p.commission / 100 * p.salesRub);
      const logisticsRub = Math.round(p.logistics / 100 * p.salesRub);
      const adsRub = Math.round(p.ads / 100 * p.salesRub);
      const expensesRub = acceptanceRub + storageRub + commissionRub + logisticsRub + adsRub;
      return {
        ...p,
        buyout,
        buyoutPct: buyout / p.sales * 100,
        orderUnits: p.sales + p.returns,
        orderRub: Math.round((p.sales + p.returns) * perUnit),
        returnsRub: Math.round(p.returns * perUnit),
        returnsPct: p.returns / p.sales * 100,
        costPct: p.cost / p.salesRub * 100,
        costUnit: Math.round(p.cost / p.sales),
        logisticsRub, storageRub, adsRub, commissionRub, acceptanceRub,
        expensesRub,
        expensesPct: expensesRub / p.salesRub * 100,
  ...
  // Standard mode: only base columns, no expanding.
  // Pro mode: base columns always; pro-extra columns appear when the group is expanded. Default collapsed.
  const visibleGroups = COLUMN_GROUPS.map(g => {
    const expanded = paid && !!collapsedGroups[g.id]; // collapsedGroups now means "expanded" set in pro
    const cols = g.cols.filter(c => c.base || (paid && expanded && c.pro));
    return { ...g, cols, expanded, hasPro: g.cols.some(c => c.pro) };
  }).filter(g => g.cols.length);
  const visibleCols = visibleGroups.flatMap(g => g.cols.map(c => ({ ...c, group: g.id })));
  ...
  const modeToggle = React.createElement('div', { style:{ display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:12, width:'100%' } },
    React.createElement('div', { style:{ display:'flex', gap:14, alignItems:'center', flexWrap:'wrap' } },
      ...
      React.createElement('span', { style:{ fontSize:12, color:'var(--text-secondary)' } },
        paid ? 'Нажмите на заголовок группы, чтобы развернуть доп. колонки' : 'Базовый набор метрик'
      )
    ),
        ...
        React.createElement('table', { style:{ width:'100%', borderCollapse:'collapse', minWidth: tableMinWidth } },
          React.createElement('thead', null,
            ...
            React.createElement('tr', { style:{ background:'var(--bg-card)' } },
              ...
              visibleGroups.map((g) =>
                React.createElement('th', {
                  key:g.id, colSpan: g.cols.length,
                  onClick: (paid && g.hasPro) ? () => toggleGroup(g.id) : undefined,
                  title: (paid && g.hasPro) ? (g.expanded ? 'Свернуть «' + g.label + '»' : 'Развернуть «' + g.label + '»') : undefined,
                  style:{ padding:'8px 12px', textAlign:'center', fontSize:12, fontWeight:700, color:'var(--text-primary)', textTransform:'uppercase', letterSpacing:.5, borderBottom:'1px solid var(--border)', borderLeft:'3px solid color-mix(in srgb, var(--text-secondary) 30%, var(--border))', cursor:(paid && g.hasPro)?'pointer':'default', userSelect:'none', whiteSpace:'nowrap' }
                },
                  React.createElement('span', { style:{ display:'inline-flex', alignItems:'center', gap:5, justifyContent:'center' } },
                    g.label,
                    (paid && g.hasPro) && React.createElement(Icon, { name: g.expanded ? 'chevronDown' : 'chevronRight', size:13, color:'var(--accent)' })
                  )
                )
              )
            ),
            ...
            React.createElement('tr', { style:{ background:'var(--bg-card)' } },
              colSlots.map((sl) => {
                const c = sl.col;
                return React.createElement(SortableHeader, { key:c.key, label:c.label, column:c.key, sortCol, sortDir, onSort:handleSort, align:'right', borderLeft: sl.firstInGroup, highlight: !!c.pro });
              })
            )
          ),
