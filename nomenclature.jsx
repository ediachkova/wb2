...
function Cell({ row, col, colStats, patterns, rank, sizeMode = false }) {
  ...
  return React.createElement('td', {
    ...
    style: {
      ...
      background: bg,
      color: status === 'bad' ? 'var(--negative)' : semanticTextColor,
      fontWeight: status === 'bad' ? 700 : (isMarginPct || isMarginRub ? 600 : 400),
    }
  },
    React.createElement('div', { style:{ display:'inline-flex', alignItems:'center', gap:6, justifyContent:'flex-end' } },
      overNorm && status !== 'bad' && React.createElement('span', { title:`Норма: ${col.norm}%`, style:{ color:'var(--warning)', fontSize:13, lineHeight:1 } }, '⚠'),
...
// ── Main report ─────────────────────────────────────────────────────────────
function NomenclatureReportV2({ paid = false }) {
  const [sortCol, setSortCol] = React.useState('salesRub');
  const [sortDir, setSortDir] = React.useState('desc');
  const [filter, setFilter] = React.useState({});
  const [expanded, setExpanded] = React.useState({});
  const [groupsExpanded, setGroupsExpanded] = React.useState({ sales:true, expenses:false, margin:true });
  const [patterns, setPatterns] = React.useState({
    heatmap: true, bars: true, threshold: true, delta: false,
    rank: true, severity: true, sparklines: paid, rowFill: true,
  });
  ...
  return React.createElement('div', { style:{ display:'flex', flexDirection:'column', gap:16 } },
    ...
    React.createElement(Card, { style:{ padding:0, overflow:'hidden' } },
      React.createElement('div', { style:{ overflowX:'auto' } },
        React.createElement('table', { style:{ width:'100%', borderCollapse:'collapse', minWidth: 240 + visibleCols.reduce((s,c) => s + c.width, 0) } },
            ...
            rows.flatMap(row => {
              ...
              return [
                React.createElement('tr', {
                  ...
                  React.createElement('td', { style:{ ...stickyStyle, padding:'10px 14px', borderBottom:'1px solid var(--border)', minWidth:280 } },
